...
public class AgentArchitecture {
	...
	public void notifyFailureForDisposedArtifactPendingOps(String artifactId) {
	   ...
	   public void runOneCycle() throws Exception {
	        ...
	        act(result);
	        checkTriggerFidelity(percepts, result);

	        CoreTuple tuple = extractor.extract(percepts, result, goalLedger);
	        ...
	        for (Percept p : percepts) groundedSource.add(p.toContextLine());
	    ...
	    public List<CoreTuple> trace() { return tupleTrace; }
	    ...
	    private void act(PlanResult result) {
	        switch (result.kind) {
	            ...
	            case INVOKE: {
	                ...
	                System.out.println("[ACT/invoke] " + artifactId + "." + operation + "(" + args + ")");
	                break;
	            }
	            ...
	            case FOCUS: {
	                ...
	                try {
	                    ...
	                    System.out.println("[ACT/focus] " + artifactId);
	                } catch (IllegalArgumentException e) {
	                    // Pre-existing gap this change would otherwise have walked straight into: refusing
	                    // STOP_OBSERVING on an always-observed artifact needs this same handling, so FOCUS
	                    // gets it too now rather than being allowed to crash the loop on a bad id.
	                    System.out.println("[ACT/focus][refused] " + e.getMessage());
	                }
	                break;
	            }
	            ...
	            case STOP_OBSERVING: {
	                ...
	                try {
	                    ...
	                    System.out.println("[ACT/stop_observing] " + artifactId);
	                } catch (IllegalArgumentException e) {
	                    System.out.println("[ACT/stop_observing][refused] " + e.getMessage());
	                }
	                break;
	            }
	        }
	    ...
	    /**
	     * Audit-only, not a hard gate — mirrors C2's status. For each pending
	     * trigger, checks whether this cycle's percepts satisfy its condition
	     * (crude keyword overlap, the same honestly-simplified style as
	     * Coherence's C1) and, if so, whether this cycle's action addresses
	     * it (its goalId matches). A satisfied-but-unaddressed trigger is
	     * flagged, not blocked — the agent may have a legitimate reason to
	     * defer or reconsider (Bratman: intentions are revisable, just not
	     * silently so), and adjudicating that is left to a human/audit
	     * process, not enforced online.
	     */
	    ...
	    /**
	     * Requires the condition's primary term — its first word, by
	     * convention the signal/event name the condition names first, e.g.
	     * "email_received" in "email_received on email-01 with sender
	     * containing greg" — to appear verbatim in a percept line. An
	     * earlier version counted any two keyword hits, which produced
	     * false positives on operation_started/operation_failed lines that
	     * incidentally mentioned the same artifact id and a substring of
	     * the sender's address without being the signal at all. This is
	     * still an approximate heuristic — it cannot distinguish two
	     * different signals sharing a primary term but different senders —
	     * flagged here rather than left silent, in the same spirit as C1.
	     */
	    private static boolean conditionSatisfied(String condition, List<Percept> percepts) {
	        ...
	        for (Percept p : percepts) {
	            if (p.toContextLine().toLowerCase().contains(primaryTerm)) return true;
	        }
	        return false;
	    }	
